...
ORCIDS = [
    ...
    "0000-0002-8434-119X",  # Lars Christian Mørch Stene
...
def abstract_tekst(w):
    """Gjenoppbygger abstractet fra OpenAlex sin 'inverted index'.

    OpenAlex lagrer abstractet som ordliste: hvert ord med posisjonene
    sine i teksten. Her settes teksten sammen igjen i riktig rekkefolge.
    """
    idx = w.get("abstract_inverted_index")
    if not idx:
        return ""
    plasser = []
    for ord_, posisjoner in idx.items():
        for pos in posisjoner:
            plasser.append((pos, ord_))
    plasser.sort()
    return " ".join(ord_ for _, ord_ in plasser)


def kutt_tekst(tekst, maks=MAKS_TEGN):
    """Korter teksten til maks tegn, uten aa klippe midt i et ord."""
    tekst = " ".join(str(tekst).split())   # rydder linjeskift og doble mellomrom
    if len(tekst) <= maks:
        return tekst
    kuttet = tekst[:maks]
    if " " in kuttet:
        kuttet = kuttet.rsplit(" ", 1)[0]
    return kuttet.rstrip(" ,.;:-") + " …"
...
def lag_rss(verk):
    ...
    ET.SubElement(kanal, "title").text = f"{SIDENAVN} - Latest Publications"
    ET.SubElement(kanal, "link").text = NETTSTED
    ET.SubElement(kanal, "description").text = (
        f"Recent publications from researchers at {SIDENAVN}.")
    ET.SubElement(kanal, "language").text = "en"
    ...
    for w in verk:
        ...
        sammendrag = kutt_tekst(abstract_tekst(w))
        if sammendrag:
            tekst += "\n" + sammendrag
